...
static void usage(FILE *out) {
  fprintf(out, "Usage: kual-next [--extensions PATH] [--model NAME] "
               "[--validate] [--version]\n"
               "  --validate  parse menus and print the resulting tree without "
               "opening FBInk\n");
}
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "--extensions") && i + 1 < argc)
      extensions = argv[++i];
      ...
      puts("kual-next " KUAL_NEXT_VERSION);
      return 0;
