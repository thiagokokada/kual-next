...
static char *condition_path(const char *cwd, const char *value) {
  return value && value[0] == '/' ? kual_xstrdup(value)
                                  : kual_join_path(cwd, value);
}
...
static char *next_token(const char **cursor, char **error) {
  ...
  static const char *const operators[] = {"-ext", "-gg!", "-gg", "-g!", "-g",
                                          "-z!",  "-e",   "-f",  "-m",  "-o",
                                          "&&",   "||",   "!"};
  if (*p != '"') {
  ...
  if (*p == '"') {
    ...
    while (*p && *p != '"') {
      ...
      if (len + 2 > cap) {
        cap *= 2;
        out = kual_xrealloc(out, cap);
      }
      out[len++] = c;
    }
    if (*p != '"') {
      free(out);
      set_error(error, "unterminated quoted token");
      return NULL;
    }
    p++;
  } else {
...
bool kual_condition_eval(const char *expr, const char *working_dir,
  ...
  while ((tok = next_token(&cursor, error_out)) != NULL) {
    if (!unary(tok) && !binary(tok)) {
      if (sp == 64) {
        set_error(error_out, "condition stack overflow");
        free(tok);
        goto fail;
      }
      stack[sp++].text = tok;
    ...
    if (unary(tok)) {
      if (sp < 1) {
        set_error(error_out, "stack underflow at %s", tok);
        free(tok);
        goto fail;
      }
      ...
      if (!strcmp(tok, "!"))
        ...
        result = has_extension(menu, x);
      else if (!strcmp(tok, "-m"))
        result = strcmp(menu->model, x) == 0;
      else {
        ...
        if (!strcmp(tok, "-e"))
          ...
          result = exists && S_ISREG(st.st_mode);
        else
          result = exists && S_ISREG(st.st_mode) && st.st_size > 0;
        free(path);
      }
      free(x);
      free(tok);
    ...
    if (!strcmp(tok, "&&"))
      ...
      if (matched == -2)
        set_error(error_out, "invalid regular expression: %s", x);
      else if (matched == -1 && tok[2] != 'g')
        set_error(error_out, "condition file not found: %s", path);
      else
        result = matched > 0;
      free(path);
    }
  ...
  if (error_out && *error_out)
    goto fail;
  if (sp != 1) {
    set_error(error_out, "condition leaves %zu values on stack", sp);
    goto fail;
  }
