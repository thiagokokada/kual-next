...
char *kual_join_path(const char *a, const char *b) {
    if (!b || !*b) return kual_xstrdup(a ? a : "");
    if (b[0] == '/') return kual_xstrdup(b);
...
char *kual_dirname(const char *path) {
    ...
    if (!slash) { free(copy); return kual_xstrdup("."); }
...
char *kual_read_file(const char *path, size_t *size_out) {
    ...
    if (end < 0 || end > 4 * 1024 * 1024L || fseek(f, 0, SEEK_SET) != 0) {
        fclose(f); errno = EFBIG; return NULL;
    }
    size_t size = (size_t)end;
    char *data = kual_xcalloc(size + 1, 1);
    if (size && fread(data, 1, size, f) != size) {
        int saved = errno; free(data); fclose(f); errno = saved ? saved : EIO;
        return NULL;
    }
    fclose(f);
...
void kual_errors_add(KualErrors *errors, const char *source, const char *format, ...) {
    if (!errors || !format) return;
    if (errors->len == errors->cap) {
        errors->cap = errors->cap ? errors->cap * 2 : 8;
        errors->items = kual_xrealloc(errors->items, errors->cap * sizeof(*errors->items));
    }
    va_list ap;
    va_start(ap, format);
    va_list copy;
    ...
    if (needed >= 0) vsnprintf(message, (size_t)needed + 1, format, ap);
    va_end(ap);
    errors->items[errors->len++] = (KualError){kual_xstrdup(source ? source : "launcher"), message};
