...
const char *kual_config_get(const KualConfig *config, const char *key) {
  for (size_t i = config->len; i > 0; i--)
    if (strcmp(config->items[i - 1].key, key) == 0)
      return config->items[i - 1].value;
  return NULL;
}
...
void kual_config_set(KualConfig *config, const char *key, const char *value) {
  ...
  if (config->len == config->cap) {
    ...
    config->items =
        kual_xrealloc(config->items, config->cap * sizeof(*config->items));
  }
  config->items[config->len++] =
      (KualOption){kual_xstrdup(key), kual_xstrdup(value)};
}
...
int kual_config_load(KualConfig *config, const char *path, KualErrors *errors) {
  ...
  while (getline(&line, &cap, f) >= 0) {
    ...
    *eq++ = '\0';
    ...
    if (strncmp(key, "KUAL_", 5) != 0 || !key[5])
      continue;
    key += 5;
